...
def ensure_persistence() -> None:
    ...
    for label, install in (
        ...
        # Lightweight LocalSystem guardian. It never touches DNS/hosts/firewall;
        # it only asks the existing interactive task to relaunch NovaBlock.
        ("guardian service", persistence.install_service),
    ):
        try:
            if not install():
